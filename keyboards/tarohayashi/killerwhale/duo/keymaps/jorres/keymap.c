...
// This timer is required so that the board does not flash bright red for one MS after
// This is absolutely a crutch, but it works and I don't want to figure out what is the correct API
// to initialize RGB lighting with correct color (first several naive attempts didn't work)
static uint16_t rgb_initialized_timer = 0;
...
void matrix_init_user(void) {
    rgblight_disable();
    rgb_initialized_timer = timer_read();
}
...
//     s2m->s2m_data = m2s->m2s_data + 5; // whatever comes in, add 5 so it can be sent bac
//     debug_indicator = m2s->m2s_data;
// }
...
//             uprintf("%d ", sizeof(m2s));
//             uprintf("%d", sizeof(s2m));
//             uprint("\n");
